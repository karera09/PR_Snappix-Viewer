...
判定を複数本（グリッド選択 / 表示中パス / 「グリッド優先・無ければ表示中」）に
散らすと、``L`` と 0-5 が別の項目に書く・キーボードで開いたメニューが別のタイルに
効く、という「無言で別の項目にユーザーデータを書く」欠陥が席ごとに出る。
この関数が唯一の判定点で、
...
def curation_subject(path: Path) -> Path:
    ...
    タイルに現れない post.md へ user_meta 行ができる）。全席に効く（右一覧の post.md 行を選んで 0-5 を
