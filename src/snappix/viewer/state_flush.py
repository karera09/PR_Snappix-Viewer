...
``viewer_state.json`` のフル保存は「GUI スレッドで値を確定 →
予算付きワーカーで突き合わせ + 書き込み」の 2 段で行う
（``ViewerWindow._persist_state_snapshot``）。**即時フラッシュ**の 5 経路
（ブックマーク追加 / 解除 / 管理ダイアログ / 検索保存 / 検索管理）と
``shared_prefs.json`` の 3 経路（テーマ / ライブラリ登録 / ライブラリ管理）も
GUI スレッドから同期 read+write を呼んではならない。到達不能な SMB では 1 回の I/O が 15〜195 秒ブロックする
