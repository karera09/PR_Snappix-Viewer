...
class SetRootResult(Enum):
    ...
    ``bool`` では、``False`` が「行き先が消えていたので**何も変えて
    いない**」と「プロンプトの『別のフォルダを開く』が内側で**別ルートへ
    遷移し、トレイルをハードリセットし終えた**」の 2 つを兼ねてしまう。判別を
    呼び出し側の ``self._root == root_before`` 比較へ外出しすると取り違えやすく、
    取り違えると ← が有効なまま残り、押すたび同じ「見つかりません」に落ちる。
...
class SearchTransition:
    ...
    Rather than two correlated ``set_root`` keyword args (``restore_search`` /
    ...
    *intent* here keeps ``set_root`` focused on swapping the root and
...
class NavEntry:
    ...
    exact position; ``mode`` is the centre-stack UI mode
    (``"browse"``/``"stage"``) at that position — a stage entry pushes the
    browse position it left, so the first 「←」 after entering the stage
    returns to the grid instead of skipping a level, and 「→」 re-enters the stage symmetrically; ``curation`` is
    the cross-library list (``"starred"`` / ``"later"``) that was showing there,
    or ``None`` for an ordinary folder position.

    ``curation`` exists so those lists are not **use-once**: without it,
    drilling into one item drops the overlay and 「戻る」 lands on the plain
    folder grid — so 「印を付けた項目を上から処理する」, the whole reason the
    lists exist, becomes a menu round-trip per item (and the filter typed on
    top of the list is lost each time).  Recording it here makes
...
class _BackForwardRouter(QObject):
    ...
    ``removeEventFilter`` の対を取り損ねると、閉じた（が破棄されていない）窓の
    ぶんまで課金され続ける。
