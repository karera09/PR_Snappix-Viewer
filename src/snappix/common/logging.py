...
class _RotatingFile:
    ...
    **ローテーションの事後条件**: *退避段
    ...
    def probe(self) -> None:
        ...
        （``viewer/app.py`` の ``except OSError`` → 案内モーダル）の
    ...
    def _rotate(self, size: int) -> None:
        ...
        **順序が肝**。「最古世代 unlink → 世代シフト → 最後に本命
        ``os.replace(current → .1)``」の順で各段の失敗を握り潰すと、本命だけが
        失敗し続ける条件（Windows で AV / インデクサが**現行ログ**を掴む —
        ローテーションのたびに開き直す現行ファイルこそ最も掴まれやすい）で、
        試行ごとに世代が 1 つずつ古い方から消え、5 回で全滅する。しかも現行
        ログは退避できないので無制限に育つ ＝「保険（過去ログ）を焼きながら、
        防ごうとした無制限成長も起きる」。
        ...
        第 2 段の要点。ここに後退が無いと、``.1`` を宛先に持つ ``os.replace``
        だけが失敗する条件（＝上記の AV が最新の過去ログを掴む条件）で
        「シフトだけが毎回 1 段回り、末尾で ``_rotate_at_bytes`` がリセット
        されるので毎バッチ再突入する」となり、本命失敗と同じ世代全滅になる。
        ``.1`` が空いた後にしか破壊が始まらないので、最後の
...
def _install_enqueue_guard() -> None:
    ...
    ``common/logging.py`` の AST ガード（``tests/`` 側）しか無く、
    プラグインが足すシンクはそれでは見えない。公開
