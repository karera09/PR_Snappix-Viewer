...
the first-run one, so a returning user doesn't read the re-prompt as a
malfunction.
...
# 訳文そのものを定数に焼かない。import 時に確定してしまうため、
# ``set_locale`` より先にこのモジュールが読み込まれると全文言が既定ロケールの
# まま固まる。他のダイアログと同じくウィジェット構築時に引くようキーだけを持つ。
...
def _terms_text_edit(text: str) -> QTextEdit:
    ...
    ``QPlainTextEdit`` renders every block with identical zero spacing, so
    the terms' numbered clauses would run straight into each other with no
    visual paragraph break and wrapped continuation lines would have no
    consistent indent. A per-block
...
class ConsentDialog(QDialog):
    ...
    By default it presents the product terms (``TERMS_TEXT``).  A caller that
    gates on a different document (a plugin's terms supplement) passes the
    already-resolved *text* / *title* / *intro* strings instead and gets the
    same consent rules — scroll-to-bottom unlock, progress hint, Esc
    swallowed, no reflexive-Enter default.  The version label belongs to the
    product terms, so it is shown only when *text* is omitted.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.setWindowTitle(title if title is not None else t(_WINDOW_TITLE_KEY))
        self.setModal(True)
        self.resize(680, 620)

        # A *previous* acceptance on file for a different version means
        ...
        self._view = _terms_text_edit(TERMS_TEXT if text is None else text)
        ...
        # must be a no-op rather than silently quitting the app (the
        # disabled 同意する soaks up the default role, so Enter does
    ...
    def keyPressEvent(self, event) -> None:  # noqa: N802 - Qt override
        ...
        # standard QDialog Esc→reject() would make the exit look like a
        # crash.  Declining stays an explicit button choice.
    ...
    def _maybe_enable(self, *_args) -> None:
        ...
        if self._agree.isEnabled():
            ...
            return
        if bar.maximum() > 0:
            # 「最後までスクロールすると押せる」とだけ言われる面に、どれだけ
            # 読んだかを示すものが無いと、長い規約では「まだなのか / もう
            # 終わりなのか」が分からない。
...
class TermsViewDialog(QDialog):
    ...
    def __init__(self, parent=None) -> None:
        ...
        # いま表示している規約の版。改定の再プロンプト（``TermsDialog``）と
        # 同じ i18n キー・同じ体裁で、あとから読み返す面にも出す。
        version_label = QLabel(
            t("common.terms_dialog.version_label", version=TERMS_VERSION)
        )
        version_label.setStyleSheet(hint_style())
...
def show_terms(parent=None) -> None:
    ...
    (a QTextDocument each) under the window until the app quits.
    """
...
def prompt_consent(parent=None, *, previous_version: str | None = None) -> bool:
    ...
    prompt was triggered by a terms revision — see ``ConsentDialog``.
    """
