"""到達不能なストレージのために GUI スレッドを塞がない後始末.
...
**ログもプロセス終了をブロックし得る**点に注意。放棄したワーカーは
daemon なので終了を止めないが、ログの実体も**同じ死んだ共有上**
（``data/logs/viewer_<pid>.log``）にあるため、素の loguru では (a) 予算超過の
警告を吐こうとした GUI スレッドが enqueue パイプが埋まった時点（実測 9
レコード）で永久ブロックし、(b) 生き残った場合も atexit ハンドラがライター
スレッドを無制限に join して「窓は閉じたのにプロセスが終わらない」へ症状が
移る。
...
class Deadline:
    ...
    挟まる」ため後者を選んでいる（共有にすると、健全な保存先でもドレインが
    予算を食い尽くしてフェーズ 2 が ``join(0)`` になる）。
    """
