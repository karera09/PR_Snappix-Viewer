...
改善しない。ビューア本体とプラグインの窓が同じガードを別々に手書きすると
片側だけが no-op のまま残りうるので、共有ヘルパにして同型の片側欠落が
構造的に起きないようにしている。
"""
...
def center_on_primary(window: QWidget, size: tuple[int, int]) -> None:
    ...
    **サイズだけ戻しても救えない**のが要点: 画面外に居るウィンドウを
    ...
    既定サイズは作業領域に**収まるようクランプ**する。例えば 1280x800 をそのまま中央寄せすると、作業領域が
    ...
    クランプは**枠（タイトルバー + 境界）の厚みを差し引いて**行う。``resize`` が決めるのはクライアント領域なので、
