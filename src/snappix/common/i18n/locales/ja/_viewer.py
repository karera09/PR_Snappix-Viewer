...
MESSAGES: dict[str, str] = {
    ...
    # チップの書式は「ラベル: 値」で統一する。ラベルは precision_label と
    ...
    # AI 側から絞り込み欄の等価トークン（種別 / 年齢区分 / 精度）を示す。
    ...
    # 既定の folder_coverage は include タグが
    # 2 群未満だと自身の無効化ルールに掛かり「開くと現在の選択肢が灰色」に
    # なる。結果は folder_strict と完全に同一なので実害は無いが、その理由を
    # 無効項目のツールチップだけに置かず、灰色の間だけ常設で出す。
    ...
    "viewer.advanced_search.examples_hint": "例: {sample}（AIタグは英語のタグ名です）",
    ...
    # 呼称は「Snappix Tagger」1 本・パス表記 (tagger/) は使わず、救済文は
    # 「AIタグの索引を作るには Snappix Tagger でスキャンしてください
    # （ヘルプ ▸ AIタグ検索のセットアップ…）」の定型へ統一。
    ...
    "viewer.advanced_search.shared_conditions_tooltip": "フィルターパネルで適用中の条件です（ここでは変更できません）。変更はツールバーの「フィルター」から。",
    ...
    # モーダル側のチェックボックスが設定タブと
    # 同じ文言だと「永続設定を編集している」と読めるが、実際は
    ...
    # 3 択の違い・速さ・効果を「モード名 —
    # 作るもの / 速さ / 効果」の 1 行 1 モードで並べる（内部用語を
    # 無定義のまま散文で並べない）。setInformativeText に入るので
    ...
    # 同じ ``build_cache_interactive`` を呼ぶ 2 つの
    # 入口（診断メニュー / 設定 ▸ キャッシュ管理）を同じ名で呼ぶ。
    ...
    # 内部識別子 ("lightbox") は表示文言と別。F11 は
    # setShortcut でメニューのキー列に載るのでラベルには併記しない。
    ...
    # フォルダの右クリックでもダブルクリックと同じ「このビューアで中を開く」を
    # 頼めるようにする。項目名は共通の common.action.open を使い、OS へ出る
    # 2 つとの差はツールチップで言う。
    ...
    # 値側の単位。ラベル（上の exif_*）と同じくカタログに置く。
    # 数値の整形はコード、語はここ。
    ...
    # 「フロア」はタガー内部用語の直訳で日本語として意味が通らないので、
    # 短ラベル + ツールチップで意味を補う。
    ...
    # 検索 0 件で「選べるものが無いのに選べ」と言わないよう、
    # 検索が空振りしている状況を名指しする専用文言へ分岐する。
    # 空状態オーケストレータの SECONDARY 文言: 空フォルダ・空
    ...
    "viewer.health_dialog.delete_aborted": "削除が途中で異常終了しました。一部は削除済みの可能性があります（再スキャンした現在の状態を表示します）",
    ...
    # カプセルの ‹ › のラベルとツールチップ。ラベル ctrl_prev / ctrl_next は
    # ステージ
    ...
    # 回転 / 反転は非破壊（ファイルは書き換えない）。操作ガイド側は
    # 「（表示のみ）」と書いているので右クリック側も揃える。
    ...
    # 全画面の中の ImageView は「入口」ではなく出口を出す
    ...
    # 上部バーは投稿タイトル（フォルダ名）のみをそのまま表示し、
    # n/m + ファイル名は常時表示カウンタが持つ。
    # -- viewer.lightbox.* -----------------------------------------------
    ...
    # ★N 併記版（未評価 = star 0 のときは counter を使う）
    ...
    # 空プレイリストの常設カード本文（1.5 秒で消えるオーバーレイと違い
    # 出したままにする面 — 手掛かりが自発的に消えないようにする）。
    # 全画面のままフォルダを開く手段は無い（このページの可視アクションは
    # [終了] だけで、投稿横断も空プレイリストでは早期 return する）ので、
    # 実行できる順序どおりに言う。
    "viewer.lightbox.empty_folder_body": "全画面表示を終了して、別のフォルダを開いてください。",
    # 非メディア表示中の F11 はフォルダ流し見へフォールバックする。
    # 「別のファイルが無言で開いた」と読まれないよう着地時に一言告げる。
    "viewer.lightbox.folder_open_notice": "先頭の画像から再生します",
    # 1 行に 1 項目ずつのヒント: 前半 = キーボード / ホイール（既定は「ホイール =
    # 前後送り」で、ズームは Ctrl+ホイール — image_wheel_zoom 既定 False と
    # 整合）、後半 = マウスユーザー向けの再表示・一覧・評価。
    # _HintOverlay は setWordWrap(True) + adjustSize なので、「 ・ 」区切りの
    # 長い行は QLabel の優先幅で折り返され 1920px でも 1 項目が途中で折れる
    # （実測: 行の実幅 1168px に対してピル幅 664px）。改行で区切れば
    # 折り返しに委ねずに 1 行に 1 項目ずつ並ぶ。
    ...
    "viewer.main_window.ai_tag_search": "AIタグ検索…",
    ...
    # キュレーション 3 軸のうちユーザータグにだけ
    # 横断の取り出し口を、スター / あとで見ると同じ 1 つの面に載せる。
    ...
    # 詳細情報ウィンドウの 2 つの検索導線は
    # 効果が背後の本窓にしか出ず、窓が重なっていると無反応に見えるので明示する。
    ...
    "viewer.main_window.filter_left_pane": "絞り込み",
    ...
    # 索引の postref 行は消されないので、リネーム / 削除後に無反応に
    # 見えないよう告げる。
    ...
    # ディレクトリスキャン完了後もサムネイル生成は
    # 続く（冷えた NAS では数分）— 残件数を出して「まだ待つべきか」に答える。
    "viewer.main_window.thumbs_pending": "サムネイル生成中… {n} 件",
    # user_meta.db を開けないと★・ユーザータグ・「あとで見る」が消える
    # （記録が全部失われたようにも見える）。理由つきで一度だけ知らせる。
    ...
    "viewer.main_window.zip_temp_not_registrable": (
        "開いている ZIP の中身は、窓を閉じると消える一時的な展開のため登録できません"
    ),
    ...
    # user_meta.db を開けない環境では上の案内が誤案内（付けても保存
    ...
    # 他の 3 見出しは「その中に何が並ぶか」を
    # 名指ししているので、ここも抽象カタカナにしない。コード側の識別子
    # (curation / set_curation / CURATION_VIEWS) は表示文言と別。
    "viewer.nav_rail.section_curation": "印を付けたもの",
    # -- viewer.perf_dialog.* --------------------------------------------
    "viewer.perf_dialog.category_summary": "カテゴリ別サマリ",
    # 「クリア」ボタンは viewer.main_window.perf_clear
    ...
    # 検索欄が同じ語を見せている間は search
    # チップを落とす（× の二重化を避ける）。その結果チップが
    # 0 本になると「件数 + すべて解除」だけの空帯に見えるので、何で
    ...
    # 「年齢制限を隠す」は永続設定で画面に痕跡が無く、
    # 部分適用中は「数が合わない」としか見えないので、件数表示に併記する。
    ...
    # キーの範囲表記は `0〜5`（全角チルダ）に統一する。ここは 0 を含めた
    ...
    # 到達不能な行のプレースホルダタイルのキャプション。missing /
    ...
    # プレースホルダタイル右クリックの張り替え導線。
    "viewer.post_grid.curation_rebind_action": "現在の場所を指定…",
    # 張り替え先に既存キュレーションがあるときの併合確認モーダル。
    ...
    # 書き込みが永続化できなかったときに成功トーストを出さない。
    # 「保存できませんでした」だけでは理由も
    # 次の一手も分からない — 利用者が自分で確かめられる 1 行を添える。
    "viewer.post_grid.curation_write_failed_toast":
        "変更を保存できませんでした: {name}\n"
        "保存先（data フォルダ）の空き容量とアクセス権を確認してください",
    # ZIP の展開先は閉じると消える一時フォルダなので、印をキーごと失う。
    "viewer.post_grid.curation_zip_temp_toast":
        "ZIP の中身には印を付けられません\n"
        "開いている ZIP の中身は、窓を閉じると消える一時的な展開です",
    ...
    # 実際には検索欄・フィルタ・AI 検索まで全次元を解除
    # するボタン — ラベルを実効果に合わせる。
    "viewer.post_grid.empty_clear_filter": "検索条件をすべて解除",
    "viewer.post_grid.empty_filtered": "絞り込みに一致する項目がありません",
    # 分野限定条件（本文・投稿タグ等）は post.md 由来
    # なので、直下に post.md を持つフォルダが無ければ必ず 0 件になる。post.md
    # が無いのは異常ではないので、事実だけを中立に述べる。
    "viewer.post_grid.empty_filtered_no_post_md": "このフォルダの直下には説明文付きの投稿がないため、「本文」などの分野限定条件には一致しません。投稿が並ぶフォルダを開いてから検索してください。",
    # 名前フィルタ 0 件の真因は多くの場合「検索範囲＝
    ...
    # ロックありのみを含む全絞り込み軸の入口。
    # 実際の絞り込み軸（★ / あとで見る / ユーザータグを含む）を列挙する。
    ...
    # 絞り込み欄の初回フォーカスで自動表示する
    # 短縮版の 2 行。全文（分野限定表・コントロール表・脚注）は「?」側に
    # 残し、ここでは誘導と**閉じ方**だけを添える（閉じ方は画面のどこかに
    # 書かれていなければならない）。
    ...
    "viewer.post_grid.filter_help_controls_heading": "<b>コントロール</b>（フィルター / AI 検索パネルの操作と等価）",
    "viewer.post_grid.filter_help_controls_heading_free": "<b>コントロール</b>（フィルターパネルの操作と等価）",
    ...
    # 欄幅に収まる短縮版。
    # 裸の「タグ」は使わない（ここで照合されるのは
    # post.md 由来の投稿タグで、tags.db 由来の AIタグではない）。
    "viewer.post_grid.filter_placeholder": "名前・タイトル・投稿タグで検索 (Ctrl+F)",
    # AI 検索が結果を持っている間、この欄は AI ヒットへの
    ...
    # 絞り込みの「あとで見る」は**読み取り条件**なのに、右クリックの
    # 書き込み命令（``watch_later_menu``）と同一文言にしない。「ロックありのみ」
    ...
    # 記号 1 文字ラベルではなく他行と同じ
    # 「ラベル + コロン」形。中立値の語も独自語ではなく
    # common.filter.all（すべて）を使う（コンボ構築側でキー参照）。
    ...
    # 中立値は common.filter.all を使う（独自語を持たない）。
    ...
    # 設定は永続するので、いま何が効いているかを
    # メニューを開かなくても席の文言で分かるようにする。
    ...
    # search_dimensions.TOKEN_FIELDS 参照）。
    ...
    # 読ませない（打ち切りと同じ原則）。最近追加一覧と
    # サブフォルダ検索が同じ 1 文言を共有する。
    "viewer.post_grid.scan_partial": "一部のフォルダを読み取れませんでした",
    "viewer.post_grid.scope_label": "検索範囲",
    # ♡ の呼称は post.md 仕様の favorites: と一致する「お気に入り数」
    # （「いいね数」とは呼ばない）。
    ...
    # ★ピッカーの各行。★グリフだけだと 4 と 5
    # を目で数えることになるので数値を併記する。
    "viewer.post_grid.star_menu_item": "{stars} ({n})",
    # 右クリックの★サブメニュー**見出し**だけの別キー。同じ右クリック
    # メニューの画像操作系は「実寸表示 (Ctrl+0)」などキーを併記するので、
    # ★も実在する 0〜5 を記載する。
    ...
    # この ☑ が制御するのは 4 種のうちサムネイル
    # 1 種だけ（アスペクト比・検索索引・フォルダ代表画像は常時 or 別設定）。
    "viewer.settings_dialog.disk_cache_label": "サムネイルの保存:",
    # 容量は即時（set_max_bytes + prune）・長辺は以後に作るサムネから効く
    # （既存分は再起動しても古いまま）ので、「再起動後に完全反映」と一括せず
    # 3 者の実態どおりに書き分ける。
    ...
    # 「グリッドのサムネイル表示上限」では何の上限かが
    # 画面から分からないので、実体（サイズスライダの上限・リスト表示時のみ
    # 実効）を言い切る表記へ。
    # 同じ物が「サイズスライダの上限」「情報パネルの
    # サイズスライダの上限」「アイコン上限」の 3 通りで呼ばない。用語表の
    # 正式名「サムネイルサイズ」＋ペイン名（グリッド / 情報パネル）へ寄せる。
    "viewer.settings_dialog.left_pane_list_max": "グリッドのサムネイルサイズの上限（リスト表示時）:",
    # Ctrl+ホイールで変えられて永続する表示
    # 設定なので、設定ダイアログにも「既定値に戻す」にも載せる。
    # 同じ「テキストプレビュー」群に text_body_max
    # =「本文読み込み上限」が並ぶため「本文」が 2 つの別対象を指さないようにする。
    ...
    # 実体は GalleryView のホイールハンドラが読む値で、
    ...
    # 「既定値に戻す」の対象範囲（表示・キャッシュ・
    # パフォーマンスの 3 タブ全部）を見せるため、確認モーダル + ツール
    # チップ + 完了トーストを出す。
    ...
    # 「ファイル一覧表示上限」は件数の上限と
    # 読めるが実体は px（アイコン寸法）。
    # 語彙を隣に並ぶ左ペイン行 (left_pane_list_max) と揃え、
    # （リスト表示時）の限定注記も同じ形で付ける。
    ...
    # clear_search_state は検索欄だけでなく検索範囲・
    ...
    # 「無効」が「設定でオフ」と「開けなかった」の
    # 2 義にならないよう言い分ける（理由の材料はコントローラが既に持っている）。
    ...
    # kept separate from the shared "common.label.display"
    # bucket — window-chrome visibility toggles vs. image-only operations would
    # be scope-mixed under one category, and that label is reused verbatim by
    ...
    "viewer.shortcuts_dialog.desc_ai_tag_search": "AIタグ検索（AI 検索パネルを開いて入力へ）",
    ...
    # Esc also exits the maximised preview
    # (desc_stage_exit below) — spell out which one wins when both could apply.
    # Esc の 3 つ目の意味（横断一覧 / 最近追加
    # 一覧からの退場 — ``post_grid._on_escape_clear`` の分岐）も表に載せる
    # （画面上いちばん大きな状態変化）。既定サイズで末尾が「…」で切れない
    # 長さに収める。
    # 文脈語も実体へ: Esc はウィンドウレベルの単一ハンドラなので
    ...
    # 分岐の全列挙を Enter 側 (desc_open_tile) に
    # 1 箇所だけ置き、こちらは参照にする（両行が完全に同一の長文を持つ必要は
    # ない — 既定サイズで末尾が切れないようにする）。
    # ドラッグ書き出しは画面上に手掛かりが
    # この 1 行しか無い。等価の手段（右クリックの項目名は
    ...
    "viewer.shortcuts_dialog.desc_focus_filter": "検索欄にフォーカス（ツールバー）",
    # ペイン間のフォーカス移動キー（畳んでいる席は
    # 移動前に開く）。フォーカスの現在位置は席の細枠で見える。
    # 副作用（畳んだ席を開く / 最大化を解く）は Python コメントではなく
    ...
    # （``FileListView`` も ``ChildrenGrid``）。文脈語は実体で書く。
    ...
    # the 1-5/0 star row appears in the
    # navigation / mode / lightbox sections — all use "{対象}にスターを設定 /
    # 0 で解除（{モード}）" so the three rows read as one template with only
    # the target/context varying (見出し語彙はフォーカス対象の表記とも整合)。
    ...
    # 表記は「ドリルダウン」(desc_drill_down と同じ —
    # main_window._on_folder_activated のコメント語彙) + その他ファイル /
    # 検索結果ファイルの2分岐を desc_drill_down と同一書式で列挙する。
    ...
    # rows for grid keys handled in
    # GalleryView.keyPressEvent (↑/↓・Home/End・PageUp/PageDown — kept in sync
    # mechanically by tests/test_viewer_shortcuts_keypress_sync.py).
    "viewer.shortcuts_dialog.desc_select_first_last": "選択を先頭 / 末尾の項目へ移動（グリッド / 一覧にフォーカス時）",
    # plain ←/→ mean something different once the
    ...
    # グリッド / 右一覧 / ナビレールはどれも ShortcutOverride を受理して
    ...
    # spell out the internal priority
    ...
    # これらは ``content_view.keyPressEvent`` が「画像ページか」だけを
    ...
    # Space / Home / End は全画面表示にしか
    # 無いと最大化プレビューとキー集合が非対称になる。両面で同じ操作にする。
    ...
    # グリッド / 右一覧の Ctrl+ホイール。画像側の desc_zoom_image と
    ...
    # いるもの」ではなくフォルダの先頭メディアへ移る（意図した機能）。
    # ボタン自身がそれを予告する。
    ...
    # post.md 本文を表示中の現在地表示。post.md は母集合の外なので
    ...
    "viewer.toolbar.mode_ai_tooltip": "AIタグで検索します（AI 検索パネルを開いてタグ入力へ）。",
    "viewer.toolbar.mode_body": "本文",
    # 単独の全文検索ダイアログは無い — 本文検索の入口はこのチップ
    ...
    # メニューバーの「表示(V)」と同名で中身が別にならないよう
    # 別名にし、アイコンのみなので中身をツールチップで列挙する。
    ...
    # サイズ probe 中の受理表示（cold NAS では
    # 展開ダイアログが出るまで数秒〜数十秒かかる）。
